...
module dguihub.layout.splitpanel;

import dguihub.core.events.event;
import dguihub.core.events.eventargs;
import dguihub.layout.layoutcontrol;
import dguihub.layout.panel;
...
class SplitPanel : LayoutControl {
   ...
   public override void updateLayout() {
      ...
      case SplitOrientation.vertical: {
            if (this._splitPos >= 0 && (this._splitPos + splitterSize) < this.width) {
               ...
               rm.resizeControl(this._panel2, this._splitPos + splitterSize, 0,
                     this.width - (this._splitPos + splitterSize), this.height);
               changed = true;
            }
         }
         break;
      ...
      default: // SplitOrientation.horizontal
      {
            if (this._splitPos >= 0 && (this._splitPos + splitterSize) < this.height) {
               ...
               rm.resizeControl(this._panel2, 0, this._splitPos + splitterSize,
                     this.width, this.height - (this._splitPos + splitterSize));
               changed = true;
            }
         }
         break;
   ...
   protected override void onPaint(PaintEventArgs e) {
      ...
      switch (this._splitOrientation) {
      case SplitOrientation.vertical: {
            c.drawEdge(Rect(this._splitPos, cr.top, splitterSize, cr.bottom),
                  EdgeType.raised, EdgeMode.left | EdgeMode.right);
      ...
      default: // SplitOrientation.horizontal
      {
            c.drawEdge(Rect(cr.left, this._splitPos, cr.right, splitterSize),
                  EdgeType.raised, EdgeMode.top | EdgeMode.bottom);
